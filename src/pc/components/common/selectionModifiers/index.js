...
export default class selectionModifiers extends Component {
  ...
  moveModule = (type) => {
    return () => {
      ...
      if (type === "down") {
        ...
        //两个数组子元素换位置
        componentJson[num] = componentJson.splice(
          num + 1,
          1,
          componentJson[num]
        )[0];
  ...
  addModule = () => {
    ...
    let {
        selectedActiveObj,
        selectedActiveObj: { num, componentJson },
      } = this.state,
      addModule = {
        component: "AddModule",
      };
    ...
    return (
      <div className="control_toolSuspension" style={info}>
        <div className="toolContent">
          <span onClick={this.reviseModule} className="font">
            编辑模块
          </span>
          <PlusOutlined
            title="添加模块"
          ...
          <ArrowDownOutlined
            title="下移模块"
            className="icon"
            onClick={this.moveModule("down")}
          />
          <ArrowUpOutlined
            title="上移模块"
            className="icon"
            onClick={this.moveModule("up")}
          />
