...
export default class specialtyChildIM extends Component {
    state = {
        componentJson: [
            {
                component: 'Banner',
                props: {
                    bannerList: [
                        { id: 1, adress: "#", src: require("../../../assets/artGrading/specialty/childIM/banner.jpg").default },
                    ]
                }
            },
            {
    ...
    componentDidMount() {
        document.title = '桔子树艺术-小升初器乐考级';
        if (!editingStatus.getState()) return false;
        //订阅 - 接收编辑器改变组件后的数据
        getChangeComponent = PubSub.subscribe('getChangeComponentData', (msg, data) => {
            if (data.type !== 'specialtyChildIM') {
    ...
    render() {
        ...
        return (
            <div>
                {
                    seekComponents(componentJson, 'specialtyChildIM')
                }
            </div>
        )
    }
