...
export default class childCaricature extends Component {
    state = {
        componentJson: [
            {
                ...
                props: {
                    introduction: {
                        img: require('../../../../assets/fineArt/nav5.jpg').default,
                        subTitle: "漫画",
                        title: "漫画介绍",
                        text: "漫画是一种艺术形式，是用简单而夸张的手法来描绘生活或时事的图画。"
                    }
                }
            },
            {
    ...
    componentDidMount() {
        document.title = '桔子树艺术-少儿漫画';

        if (!editingStatus.getState()) return false;
        //订阅 - 接收编辑器改变组件后的数据
        getChangeComponent = PubSub.subscribe('getChangeComponentData', (msg, data) => {
            if (data.type !== 'childCaricature') {
    ...
    render() {
        ...
        return (
            <div>
                {
                    seekComponents(componentJson, 'childCaricature')
                }
            </div>
        )
    }
