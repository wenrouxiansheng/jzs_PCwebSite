...
export default class adultPencilSketch extends Component {
    state = {
        componentJson: [
            {
                ...
                props: {
                    introduction: {
                        img: require('../../../../assets/fineArt/nav1.jpg').default,
                        subTitle: "素描",
                        title: "素描介绍",
    ...
    componentDidMount() {
        document.title = '桔子树艺术-成人素描';

        if (!editingStatus.getState()) return false;
        //订阅 - 接收编辑器改变组件后的数据
        getChangeComponent = PubSub.subscribe('getChangeComponentData', (msg, data) => {
            if (data.type !== 'adultPencilSketch') {
    ...
    render() {
        ...
        return (
            <div>
                {
                    seekComponents(componentJson, 'adultPencilSketch')
                }
            </div>
        )
    }
