...
export default class adultSketch extends Component {
    state = {
        componentJson: [
            {
                ...
                props: {
                    introduction: {
                        img: require('../../../../assets/fineArt/sketch/nav2.jpg').default,
                        subTitle: "速写",
                        title: "速写介绍",
    ...
    componentDidMount() {
        document.title = '桔子树艺术-成人速写';

        if (!editingStatus.getState()) return false;
        //订阅 - 接收编辑器改变组件后的数据
        getChangeComponent = PubSub.subscribe('getChangeComponentData', (msg, data) => {
            if (data.type !== 'adultSketch') {
    ...
    render() {
        ...
        return (
            <div>
                {
                    seekComponents(componentJson, 'adultSketch')
                }
            </div>
        )
    }
