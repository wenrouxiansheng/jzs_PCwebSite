...
export default class musicBasis extends Component {
    state = {
        componentJson: [
            {
                component: 'Banner',
            ...
            },
            {
                ...
                props: {
                    introduction: {
                        img: require('../../../../assets/music/childMusic/nav2.jpg').default,
                        subTitle: "少儿声乐（童声）",
                        title: "少儿声乐（童声）",
    ...
    componentDidMount() {
        document.title = '桔子树艺术-少儿音基';

        if (!editingStatus.getState()) return false;
        //订阅 - 接收编辑器改变组件后的数据
        getChangeComponent = PubSub.subscribe('getChangeComponentData', (msg, data) => {
            if (data.type !== 'musicBasis') {
    ...
    render() {
        ...
        return (
            <div>
                {
                    seekComponents(componentJson, 'musicBasis')
                }
            </div>
        )
    }
