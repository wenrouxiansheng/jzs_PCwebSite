...
export default class solfeggio extends Component {
    state = {
        componentJson: [
            {
                ...
                props: {
                    introduction: {
                        img: require('../../../../assets/music/childMusic/nav3.jpg').default,
                        subTitle: "少儿声乐（童声）",
                        title: "少儿声乐（童声）",
    ...
    componentDidMount() {
        document.title = '桔子树艺术-视唱练耳';

        if (!editingStatus.getState()) return false;
        //订阅 - 接收编辑器改变组件后的数据
        getChangeComponent = PubSub.subscribe('getChangeComponentData', (msg, data) => {
            if (data.type !== 'solfeggio') {
    ...
    render() {
        ...
        return (
            <div>
                {
                    seekComponents(componentJson, 'solfeggio')
                }
            </div>
        )
    }
